...
use common\models\CaptchaMobile;
use common\models\UserLogs;
 ...
 * @since 2.0
 */
class CaptchaMobileAction extends Action
{
    /**
    ...
    public function run($mobile)
    {
        ...
        $code1 = $this->getVerifyCode(true);

        $count = CaptchaMobile::find()->andWhere(['uid' => Yii::$app->session->get("user_id")])->andWhere("created_at>=:start and created_at<:end", [':start' => date("Y-m-d 00:00:00"), ':end' => date("Y-m-d 23:59:59")])->count();

        if ($count >= Yii::$app->params['message_count']) {
            return $controller->returnJson([], false, "发送验证码过于频繁");
        }

        $captchaImages = new CaptchaMobile();
        $captchaImages->uid = Yii::$app->session->get("user_id");
        $captchaImages->code = (string)$code1;
        $captchaImages->mobile = $mobile;
        $captchaImages->ip = CommonHelper::ip();
        $captchaImages->expired_at = date("Y-m-d H:i:s", strtotime("+30 minutes"));
        $captchaImages->save();


        $userLogs = new UserLogs();
        $userLogs->uid = Yii::$app->session->get("user_id");
        $userLogs->ip = CommonHelper::ip();
        $userLogs->action = "发送手机验证码";
        $userLogs->save();

        $response = \Requests::post(Yii::$app->params['captcha_host'], [], json_encode([
            'target' => $mobile,
            'content' => strtr(Yii::$app->params['captcha_content'], [
                '{0}' => $code1,
            ])
        ]));
